...
- feat: tone pool expansion

## 2.6.16

- 梦境系统 v3：防泄漏闸与视角修正。v2 的 full 档验收产物出现三处缺陷——第三段退化为词表罗列、具名短语抽取失控导致桶/暗房原文以整句级泄漏进梦文（如"她说她心智健全，人格完整"）、生成全程第三人称未被第一人称指令纠正。
  - 新增 n-gram 防泄漏闸（改动一，最高优先）：落盘前对全部 source（桶原文+暗房底片原文，不含噪音词）做连续字符匹配，≥`leak_ngram`（默认 10）字重合即判泄漏，整发判废重试一次，二次泄漏按无梦处理不落盘；日志只记重合长度，不记内容（R4）。
  - 具名短语硬约束（改动二）：extract prompt 强化"不得是完整句子、不含任何标点"；代码层兜底丢弃超过 12 字或含句读符号的短语，不截断硬凑；暗房底片参与拆意象时只出意象词，不出具名短语。
  - `_is_prose_like()` 改回逐段判定（改动三）：v2 的全文密度判定会放行"整体够、局部退化成词表"的产物；v3 用 jieba 词性标注区分合法混沌短句（含动词/完整场景描述）和非法词表（连续 ≥3 个无动词纯名词片段顿号/逗号串联），jieba 不可用时退化为长度启发式。
  - 第一人称硬化（改动四）：两套生成 prompt 第一行改为视角指令+正例；代码层校验全文"我"少于 2 次或首句以"她/他"为主语开头即判违规。
  - 泄漏闸/词表判定/第一人称校验统一走同一条重试逻辑（最多重试 1 次，二次失败按无梦处理不落盘）。新增 20+ 个结构测试；真实产物验收在部署环境用 `DREAM_FORCE=1` + `DREAM_FORCE_LEVEL` 做，本地无 API key 不碰真调用。

## 2.6.15

- 梦境系统 v2：清晰/混沌交替结构，依据真实做梦节奏（"一段很清晰的情节，混一段乱七八糟记不清的，再来一段清晰的，又跟一大段乱七八糟的"）重构高记忆度档的生成质量。
  - 管线顺序调整：记忆度骰前移到生成步之前（含"只剩情绪"档 70% 基调改判），生成步现在知道自己在写哪一档。
  - 拆意象额外抽取最多 1 条具名短语（含人名/称呼/专有名词，桶内无专名不硬凑），单独传给生成步当"清晰段的锚"。
  - 生成 prompt 按档分两套：完全记得/记得一半用新的清晰段（80-150字，允许情节连贯与因果）+ 混沌段（意象并置，禁因果）交替结构，共 4-6 段，300-600 字，max_tokens 上调至 1200；只剩画面/只剩情绪维持 v1 prompt 不变。
  - 记得一半档裁剪改为按段落粒度（保留连续整段，约占全文 45%-60%，超长段截段内窗口），模型未按空行分段时退化为原字符窗口裁法。
  - 新增 `DREAM_FORCE_LEVEL` 测试变量（full/half/scene/emotion，仅 `DREAM_FORCE=1` 时生效），跳过记忆度骰强制指定档位，基调骰与联动改判照常。
  - `_is_prose_like()` 判定粒度从逐段/逐行改为全文句读密度，避免交替结构里短促无标点的混沌段被误判为清单体。
  - 新增 16 个结构测试覆盖以上改动；真实产物验收在部署环境进行（`DREAM_FORCE=1` + `DREAM_FORCE_LEVEL` 强制生成，本地无 API key 不碰真调用）。

## 2.6.14

- 修复梦境系统首发实测发现的生成质量问题：生产上 `dreams/2026-07-31.md` 正文是意象词原样罗列（清单体），不是叙事散文。排查确认管线里没有任何退化路径会把词表落盘——问题出在 §1.6 生成 prompt 本身没有明确要求"输出连续散文"，配合换行拼接的词表输入、1.3 高 temperature、便宜小模型，模型直接顺着输入的清单形状续写了回去。修复：①重写生成 prompt，明确要求连续散文、每句含动词、意象词是素材不是输出格式，禁止分行罗列；②user 消息由换行拼接改为顿号拼接，避免输入本身就是清单形状；③新增 `_is_prose_like()` 结构校验作为兜底——生成结果形似清单体时一律按生成失败处理，不落盘（只记段数/字数统计，不记正文，符合 R4）。新增 5 个回归测试复现原始 bug 形状。
